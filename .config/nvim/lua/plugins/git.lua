  ...
  vim.api.nvim_create_autocmd('FileType', {
    ...
    callback = function(_)
      ...
      vim.keymap.set('n', 'cS', function()
        ...
        return [[<Cmd>silent !tmux new-window 'git commit --squash ]] ..
            sha ..
            [[ && git rebase -i --autosquash ]] .. sha .. [[^; read -n 1 -s -p "press any key to close ..."'<CR>]]
      end, { buffer = 0, expr = true })
  ...
  return {
    ...
    keymaps = {
      ...
      file_panel = {
        ...
        { 'n', 'cC',     '<Cmd>tabclose <bar> silent !tmux new-window \'git commit -n; read -n 1 -s -p "press any key to close ..."\'<CR>',                   { desc = 'Commit no verify' } },
        { 'n', 'ca',     '<Cmd>tabclose <bar> silent !tmux new-window \'git commit --amend; read -n 1 -s -p "press any key to close ..."\'<CR>',              { desc = 'Commit amend' } },
        { 'n', 'cA',     '<Cmd>tabclose <bar> silent !tmux new-window \'git commit --amend -n; read -n 1 -s -p "press any key to close ..."\'<CR>',           { desc = 'Commit amend no verify' } },
        ...
        { 'n', 'cE',     '<Cmd>tabclose <bar> silent !tmux new-window \'git commit --amend --no-edit -n; read -n 1 -s -p "press any key to close ..."\'<CR>', { desc = 'Commit amend no edit no-verify' } },
        { 'n', '<F9>',   '<Cmd>tabclose <bar>GV --all<CR>',                                                                                                   { desc = 'Open the commit log' } },
        { 'n', '<S-F9>', '<Cmd>tabclose <bar>GV --name-status --all<CR>',                                                                                     { desc = 'Open the commit log --name-status' } },
