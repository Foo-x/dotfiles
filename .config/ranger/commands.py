...
# load executables
threading.Thread(target=get_executables).start()
...
class fzf_select(Command):
    ...
    def execute(self):
        ...
        env['FZF_DEFAULT_OPTS'] = '--reverse --cycle --bind "change:top" --ansi --preview="{}"'.format('''
            (
                batcat --color=always {} ||
                bat --color=always {} ||
                cat {} ||
...
class fzf_mark(Command):
    ...
    def execute(self):
        ...
        --reverse \
        --cycle \
        --bind ctrl-a:select-all,ctrl-e:deselect-all,alt-n:down,alt-p:up,alt-o:backward-delete-char,alt-h:beginning-of-line,alt-l:end-of-line,alt-j:backward-char,alt-k:forward-char,alt-b:backward-word,alt-f:forward-word,change:top \
        --layout reverse \
        --border \
        --preview "(bat --color=always {} || cat {}) 2> /dev/null | head -n 100"'
...
class fzf_search(Command):
    ...
    def execute(self):
        ...
        if not self.arg(1):
            self.fm.notify('Usage: fzf_search <search string>', bad=True)
            return
        ...
        if rg is not None and fzf is not None and bat is not None:
            ...
            fzf = self.fm.execute_command(
                f'rg --hidden -g \'!.git\' -l "{self.rest(1)}" | fzf --reverse --cycle --bind "change:top" --ansi --preview "bat --color=always {{}}"',
                universal_newlines=True,stdout=subprocess.PIPE)
