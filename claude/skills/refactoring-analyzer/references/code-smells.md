...
### Redundant Comment（冗長なコメント）
**定義**: コードを読めば自明な内容をそのまま説明しているコメント。
**検出基準**: 変数への代入や1行の処理をそのまま日本語化しただけの記述、命名から明らかな内容の繰り返し
**リファクタリング手法**: Remove Redundant Comment、Extract Method、Rename Method

### Commented-Out Code（コメントアウトされたコード）
**定義**: 使われなくなった旧コードがコメントとして残存している。
**検出基準**: `//`や`/* */`で無効化されたコード片、実行されないコードブロック
**リファクタリング手法**: Remove Commented-Out Code

### Outdated / Misleading Comment（陳腐化・誤誘導コメント）
**定義**: コメントの内容が実際のコードの振る舞いと食い違っている。
**検出基準**: コメントが説明するロジックと実装が一致しない、変更履歴上コードだけが更新されコメントが放置されている
**リファクタリング手法**: 更新または削除（Remove Redundant Comment）

### Comment Compensating for Bad Code（悪いコードを補うコメント）
**定義**: 分かりにくい命名や構造をコメントで説明で補っている。
**検出基準**: 曖昧な変数名・関数名の直後に長い説明コメント、複雑な条件式へのコメントによる注釈
**リファクタリング手法**: Extract Method、Rename Method、Introduce Assertion

### Task / Context Reference Comment（タスク・文脈参照コメント）
**定義**: 現在のタスクやissue、呼び出し元など変更されやすい文脈への言及。
**検出基準**: 「〇〇機能のために追加」「issue #123の対応」などのコメント
**リファクタリング手法**: 削除（文脈はPR・コミットメッセージに記載）
